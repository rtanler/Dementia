...
- **Explore mode:** Click any cell to open a detail panel below the table
  with a longer explanation of that characteristic for the selected type.
  The table header and left column are sticky so they stay visible when
  scrolling.
- **Quiz mode:** Read a short clinical scenario and pick the most likely
  dementia type. You get immediate feedback with an explanation. Five
  scenarios cover all five types. Getting all five correct triggers a
  celebration.
